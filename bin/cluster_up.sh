...
do
    ...
    taskset -c $cpu bash -c "PORT=$((8080 + $i)) cargo run > \"$logDir/app_instance_$i.log\" 2>&1 &"
    echo "App instance $i started on port $((8080 + $i)), assigned to CPU $cpu, and logging to $logDir/app_instance_$i.log"
done
